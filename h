...
local SectionESP1 = TabESP:CreateSection("视觉效果")
local SectionESP2 = TabESP:CreateSection("玩家信息")
local SectionESP3 = TabESP:CreateSection("过滤器")
local SectionESP4 = TabESP:CreateSection("骨骼")

local TabMBot = Window:CreateTab("自瞄")
local SectionMBot = TabMBot:CreateSection("自瞄设置")
local SectionTrigger = TabMBot:CreateSection("自动开火")
local SectionFire = TabMBot:CreateSection("射击修改")

local TabHVH = Window:CreateTab("HVH")
local SectionHVH1 = TabHVH:CreateSection("战斗")
local SectionHVH2 = TabHVH:CreateSection("移动")
local SectionHVH3 = TabHVH:CreateSection("视觉")

local TabExtra = Window:CreateTab("额外功能")
local SectionExtra1 = TabExtra:CreateSection("移动")
local SectionExtra2 = TabExtra:CreateSection("传送")
local SectionExtra3 = TabExtra:CreateSection("其他")
        ...
        if character and character:FindFirstChild("Humanoid") and character:FindFirstChild("Head") then
            ...
            if drawings.HealthText then 
                ...
                if drawings.HealthText.Enabled then
                    drawings.HealthText.TextLabel.Text = string.format("%.0f 生命值", humanoid.Health)
                end
            end
            ...
            if drawings.WeaponText then 
                ...
                if drawings.WeaponText.Enabled then
                    ...
                    drawings.WeaponText.TextLabel.Text = tool and tool.Name or "无武器"
                end
            end
            ...
            if drawings.DistanceTag and drawings.DistanceTag.Enabled then
                ...
                drawings.DistanceTag.TextLabel.Text = string.format("%.0f 单位", distance)
            end
...
-- Toggles ESP
SectionESP1:CreateToggle("2D 方框", nil, function(State) 
...
SectionESP2:CreateToggle("名称", nil, function(State) 
...
SectionESP2:CreateToggle("距离", nil, function(State) 
...
SectionESP2:CreateToggle("生命条", nil, function(State) 
...
SectionESP2:CreateToggle("生命值文本", nil, function(State) 
...
SectionESP2:CreateToggle("武器", nil, function(State) 
...
SectionESP1:CreateToggle("彩虹透视", nil, function(State) 
    ESP_Toggles.Rainbow = State 
end)

SectionESP1:CreateToggle("透明", nil, function(State) 
...
SectionESP1:CreateToggle("实体透视", nil, function(State) 
...
SectionESP1:CreateToggle("轮廓", nil, function(State) 
...
SectionESP4:CreateToggle("骨骼", nil, function(State) 
...
SectionESP4:CreateToggle("射线", nil, function(State) 
...
SectionESP3:CreateToggle("仅敌人", nil, function(State)
    ESP_Toggles.EnemiesOnly = State
end)

SectionESP1:CreateButton("重置透视", function()
    resetESP()
end)
...
-- Options Aimbot
SectionMBot:CreateToggle("自瞄", nil, function(State) 
...
SectionMBot:CreateToggle("显示 FOV", nil, function(State) 
    Aimbot_Toggles.ShowFOV = State 
end)

SectionMBot:CreateDropdown("瞄准部位", {"头部", "上半身", "身体根部"}, function(String)
    if String == "头部" then
        Aimbot_Toggles.AimPart = "Head"
    elseif String == "上半身" then
        Aimbot_Toggles.AimPart = "UpperTorso"
    elseif String == "身体根部" then
        Aimbot_Toggles.AimPart = "HumanoidRootPart"
    end
end)

SectionMBot:CreateSlider("FOV 大小", 50, 500, 200, true, function(Value)
    Aimbot_Toggles.FOVSize = Value
end)

SectionMBot:CreateSlider("平滑度", 1, 10, 1, true, function(Value)
    Aimbot_Toggles.Smoothness = Value
end)

SectionMBot:CreateToggle("队伍检查", nil, function(State)
    Aimbot_Toggles.TeamCheck = State
end)

SectionMBot:CreateToggle("预判", nil, function(State)
    Aimbot_Toggles.Prediction = State
end)

SectionMBot:CreateSlider("预判量", 0.1, 0.5, 0.14, false, function(Value)
...
-- Options Trigger Bot
SectionTrigger:CreateToggle("自动开火", nil, function(State)
...
end):AddToolTip("当敌人在 FOV 内时自动开火")

SectionTrigger:CreateSlider("开火延迟", 0, 1, 0.1, false, function(Value)
    Aimbot_Toggles.TriggerDelay = Value
end)

-- Options Fire Modifiers
SectionFire:CreateToggle("快速射击 ⚡", nil, function(State)
...
end):AddToolTip("修改武器的射速")

SectionFire:CreateToggle("高速射击", nil, function(State)
...
end):AddToolTip("提高射击速度")

SectionFire:CreateToggle("无射击延迟 🔥", nil, function(State)
...
-- Options HVH
SectionHVH1:CreateToggle("修改 FOV", nil, function(State)
    HVH_Toggles.ChangeFOV = State
    updateFOV()
end)

SectionHVH1:CreateSlider("FOV 值", 70, 150, 120, true, function(Value)
    HVH_Toggles.FOVValue = Value
    updateFOV()
end)

SectionHVH1:CreateToggle("反自瞄", nil, function(State)
...
end):AddToolTip("使你的命中框不同步")

SectionHVH1:CreateToggle("不同步", nil, function(State)
    HVH_Toggles.Desync = State
end)

SectionHVH1:CreateSlider("不同步量", 0.1, 2, 0.5, false, function(Value)
    HVH_Toggles.DesyncAmount = Value
end)

SectionHVH1:CreateToggle("旋转机器人", nil, function(State)
...
SectionHVH1:CreateSlider("旋转速度", 1, 20, 5, true, function(Value)
    HVH_Toggles.SpinSpeed = Value
end)

SectionHVH1:CreateToggle("假延迟", nil, function(State)
...
end):AddToolTip("延迟你的位置以欺骗敌人")

SectionHVH1:CreateSlider("假延迟 (毫秒)", 50, 500, 100, true, function(Value)
    HVH_Toggles.FakeLagAmount = Value
end)

SectionHVH2:CreateToggle("速度黑客", nil, function(State)
...
end):AddToolTip("增加你的移动速度")

SectionHVH2:CreateSlider("速度倍数", 1, 5, 2, true, function(Value)
...
SectionHVH3:CreateToggle("第三人称", nil, function(State)
    HVH_Toggles.ThirdPerson = State
    toggleThirdPerson()
end):AddToolTip("第三人称视角")

SectionHVH3:CreateSlider("第三人称距离", 5, 20, 10, true, function(Value)
...
-- Options Extra
SectionExtra1:CreateToggle("飞行", nil, function(State)
    Extra_Toggles.Fly = State
    if State then startFly() else stopFly() end
end)

SectionExtra1:CreateSlider("飞行速度", 1, 100, 50, true, function(Value)
    Extra_Toggles.FlySpeed = Value
end)

SectionExtra1:CreateToggle("穿墙", nil, function(State)
...
SectionExtra1:CreateToggle("速度", nil, function(State)
...
SectionExtra1:CreateSlider("行走速度", 16, 100, 25, true, function(Value)
...
SectionExtra2:CreateToggle("传送到玩家", nil, function(State)
...
end):AddToolTip("传送到最近的敌人")

SectionExtra3:CreateToggle("无限跳跃", nil, function(State)
...
SectionExtra3:CreateToggle("全亮", nil, function(State)
...
SectionExtra3:CreateToggle("无后坐力", nil, function(State)
...
SectionExtra3:CreateToggle("无扩散", nil, function(State)
...
-- UI Toggle
SectionESP1:CreateToggle("UI 切换", nil, function(State)
    Window:Toggle(State)
