...
-- ▼▼▼ Add to the utilities module that returns `g` (same place as g.Tween / g.Image etc.) ▼▼▼
function g.StartSnow(opts)
    -- options with sane defaults
    opts = opts or {}
    local density     = tonumber(opts.density)     or 70      -- 雪花数量
    local minSize     = tonumber(opts.minSize)     or 10      -- 像素
    local maxSize     = tonumber(opts.maxSize)     or 26
    local minSpeed    = tonumber(opts.minSpeed)    or 40      -- 垂直速度(px/s)
    local maxSpeed    = tonumber(opts.maxSpeed)    or 120
    local driftAmp    = tonumber(opts.driftAmp)    or 30      -- 左右漂移幅度(px)
    local driftFreq   = tonumber(opts.driftFreq)   or 0.6     -- 漂移频率(Hz)
    local spinMin     = tonumber(opts.spinMin)     or -20     -- 旋转速度(°/s)
    local spinMax     = tonumber(opts.spinMax)     or  40
    local zIndex      = tonumber(opts.zIndex)      or 999999  -- 盖在最上
    local parentGui   = (opts.parent and opts.parent:IsA("GuiObject")) and opts.parent
                        or (g.WindUI and g.WindUI.ScreenGui)           -- 自动挂到 WindUI 的 ScreenGui
                        or error("StartSnow: no valid parent gui")

    -- 如果已开启，先跳过
    if g.__Snow and g.__Snow.running then return g.__Snow end

    -- 容器
    local container = g.New("Frame", {
        Name = "__SnowLayer",
    ...
    -- 工具：生成一个雪花（使用 Unicode 字符 ❆，避免外链贴图）
    local function newFlake()
        ...
        local xStart = rng:NextNumber(-0.05, 1.05) -- 宽度百分比（支持从边外进来）
        local yStart = rng:NextNumber(-0.15, -0.02)

        local label = g.New("TextLabel", {
            Size = UDim2.fromOffset(size, size),
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = UDim2.new(xStart, 0, yStart, 0),
            BackgroundTransparency = 1,
            Text = "❆", -- 也可以换成 "✽" "✦" "❄"
            RichText = false,
            TextScaled = true,
            FontFace = Font.new(g.Font or "rbxassetid://12187365364", Enum.FontWeight.Medium),
            ThemeTag = { TextColor3 = "Text" }, -- 跟随主题文字颜色
            Parent = container,
        })

        -- 给每片雪一个轻微透明随机
        label.TextTransparency = rng:NextNumber(0.05, 0.25)
    ...
    -- 预生成
    for _ = 1, density do
        table.insert(flakeList, newFlake())
    end
    ...
    conn = heartbeat:Connect(function(dt)
        if not running or not container or not container.Parent then
        ...
        for i = 1, #flakeList do
            local flake = flakeList[i]
            local ui    = flake.ui
            if not ui or not ui.Parent then
                -- 若被外部移除，补一片
                flakeList[i] = newFlake()
                flake = flakeList[i]
                ui = flake.ui
            end

            -- 读当前位置（用像素便于物理感）
            local xPx = ui.AbsolutePosition.X + ui.AbsoluteSize.X * 0.5 - container.AbsolutePosition.X
            local yPx = ui.AbsolutePosition.Y + ui.AbsoluteSize.Y * 0.5 - container.AbsolutePosition.Y

            -- 垂直下落
            yPx = yPx + flake.speed * dt

            -- 左右漂移：sin 波 + 相位
            local drift = math.sin((time + flake.phase) * (math.pi * 2) * driftFreq) * driftAmp * dt
            xPx = xPx + drift

            -- 旋转
            ui.Rotation = ui.Rotation + flake.spin * dt

            -- 归一化回 GUI 坐标
            local xScale = math.clamp(xPx / math.max(absSize.X, 1), -0.1, 1.1)
            local yScale = yPx / math.max(absSize.Y, 1)

            ui.Position = UDim2.new(xScale, 0, yScale, 0)

            -- 超出底部 -> 重生到顶部
            if yScale > 1.1 then
                local newSize = rng:NextInteger(minSize, maxSize)
                ui.Size = UDim2.fromOffset(newSize, newSize)
                ui.TextTransparency = rng:NextNumber(0.05, 0.25)

                flake.speed = rng:NextNumber(minSpeed, maxSpeed)
                flake.phase = rng:NextNumber(0, math.pi*2)
                flake.spin  = rng:NextNumber(spinMin, spinMax)

                local xStart = rng:NextNumber(-0.05, 1.05)
                ui.Position = UDim2.new(xStart, 0, rng:NextNumber(-0.15, -0.02), 0)
                ui.Rotation = rng:NextNumber(-15, 15)
            end
        end
    end)
...
function g.StopSnow()
    if g.__Snow then g.__Snow:stop() g.__Snow = nil end
end
-- ▲▲▲ End of snow util
